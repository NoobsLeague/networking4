...
public class LobbyState : ApplicationStateWithView<LobbyView>
{
    ...
    private void onTextEntered(string pText)
    {
        view.ClearInput();
        //yay chat
        fsm.channel.SendMessage(new ChatMessage { message = pText });
        //bruh
    ...
    protected override void handleNetworkMessage(ASerializable msg)
    {
        if (msg is ChatMessage chat)
            view.AddOutput(chat.message);
        else if (msg is LobbyInfoUpdate info)
            view.SetLobbyHeading($"Lobby ({info.memberCount} here, {info.readyCount} ready)");
        else if (msg is RoomJoinedEvent rje && rje.room == RoomJoinedEvent.Room.GAME_ROOM)
